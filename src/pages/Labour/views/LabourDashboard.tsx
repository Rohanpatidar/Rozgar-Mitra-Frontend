...
import { bookingApi, type LabourStats, type NearbyTask } from '../../../api/booking.api';
...
export const LabourDashboard: React.FC = () => {
    ...
    const [stats, setStats] = useState<LabourStats>({ acceptedTasks: 0, todayEarnings: 0 });
    ...
    useEffect(() => {
        Promise.all([apiGet<boolean>('/labour/availability'), apiGet<LabourStats>('/labour/stats')])
            .then(([availability, statsResponse]) => {
                setIsOnline(availability.data);
                ...
                <Grid size={{ xs: 12, sm: 6 }}>
                    <Paper elevation={0} sx={{ p: 2, borderRadius: '16px', border: '1px solid #e2e8f0', textAlign: 'center' }}>
                        ...
                        <Typography variant="h5" sx={{ fontWeight: 800, color: LABOUR_THEME.primary, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <CurrencyRupeeIcon fontSize="small" /> {stats.todayEarnings.toFixed(2)}
                        </Typography>
                    </Paper>
                </Grid>
                <Grid size={{ xs: 12, sm: 6 }}>
                    <Paper elevation={0} sx={{ p: 2, borderRadius: '16px', border: '1px solid #e2e8f0', textAlign: 'center' }}>
                        ...
                        <Typography variant="h5" sx={{ fontWeight: 800, color: LABOUR_THEME.text }}>{stats.acceptedTasks}</Typography>
                    </Paper>
                </Grid>
            </Grid>
